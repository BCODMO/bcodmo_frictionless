...
from dataflows import Flow, update_resource, PackageWrapper
...
# Aggregators
Aggregator = collections.namedtuple(
    "Aggregator", ["func", "finaliser", "dataType", "copyProperties"]
)
AGGREGATORS = {
    "sum": Aggregator(
        lambda curr, new: new + curr if curr is not None else new, identity, None, False
    ),
    "avg": Aggregator(
        lambda curr, new: (curr[0] + 1, new + curr[1])
        if curr is not None
        else (1, new),
    ...
    "median": Aggregator(
        lambda curr, new: curr + [new] if curr is not None else [new],
        median,
        None,
        True,
    ),
    "max": Aggregator(
        lambda curr, new: max(new, curr) if curr is not None else new,
        identity,
        None,
        False,
    ),
    "min": Aggregator(
        lambda curr, new: min(new, curr) if curr is not None else new,
        identity,
        None,
        False,
    ),
    "first": Aggregator(
        lambda curr, new: curr if curr is not None else new, identity, None, True
    ),
    "last": Aggregator(lambda curr, new: new, identity, None, True),
    "count": Aggregator(
        lambda curr, new: curr + 1 if curr is not None else 1,
        identity,
        "integer",
        False,
    ),
    "any": Aggregator(lambda curr, new: new, identity, None, True),
    "set": Aggregator(
        lambda curr, new: curr.union({new}) if curr is not None else {new},
    ...
    "counters": Aggregator(
        lambda curr, new: update_counter(curr, new),
        lambda value: list(collections.Counter(value).most_common())
...
def join_aux(
    ...
    def process_target_resource(source_spec, resource):
        target_fields = resource.setdefault("schema", {}).setdefault("fields", [])
        for name, spec in fields.items():
            ...
            try:
                ...
                ), "Reusing %s but with different data types: %s != %s" % (
                    name,
                    existing_field["type"],
                    data_type,
                )
    ...
    def process_datapackage(datapackage):
        ...
        assert (
            source_name in resource_names
        ), "Source resource ({}) not found package (target={}, found: {})".format(
            source_name, target_name, resource_names
        )
        assert (
            target_name in resource_names
        ), "Target resource ({}) not found package (source={}, found: {})".format(
            target_name, source_name, resource_names
        )
        ...
        for resource in datapackage["resources"]:
            if resource["name"] == source_name:
                ...
                assert isinstance(
                    source_spec, dict
                ), "Source resource ({}) must appear before target resource ({}), found: {}".format(
                    source_name, target_name, resource_names
                )
...
def join_with_self(resource_name, join_key, fields):
    return join_aux(
        resource_name, join_key, True, resource_name, None, fields, True, None
    )
...
def join_self(source_name, source_key, target_name, fields):
    ...
    warnings.warn(
        "join_self is being deprecated, use join_with_self instead", DeprecationWarning
    )
    return join_aux(
        source_name, source_key, True, target_name, None, fields, True, None
    )
